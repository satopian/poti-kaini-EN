...
function PaletteSet(){var a=document.forms.Palette;const b=a.setr.value,c=a.select;var e=a.m_m.selectedIndex;a=b.length;let f;if(1>a)alert("There is no matrix information.");else{var d;switch(e){default:for(d=c.length;0<d;)d--,c.options[d]=null;case 2:e=c.options.length;d=b.indexOf("!",0)+1;if(0==d)return;for(;d<a;){var g=b.indexOf("\n#",d);if(-1==g)return;const h=b.substring(d,g+1);d=b.indexOf("!",g);if(-1==d)return;f=b.substring(g+1,d+-1);"Palette"!=h?(0<=e&&(c.options[e]=new Option(h)),Palettes[e]=
f,e++):document.paintbbs.setColors(f);d+=1}break;case 1:d=b.indexOf("!",0)+1;if(0==d)return;g=b.indexOf("\n#",d);d=b.indexOf("!",g);0<=g&&(f=b.substring(g+1,d-1));document.paintbbs.setColors(f)}PaletteListSetColor()}}function PaletteListSetColor(){let a;const b=document.forms.Palette.select;for(a=1;b.options.length>a;a++){var c=Palettes[a].split("\n");b.options[a].style.background=c[4];b.options[a].style.color=GetBright(c[4])}}
